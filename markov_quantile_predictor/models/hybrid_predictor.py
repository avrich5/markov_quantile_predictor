...
class EnhancedHybridPredictor(MarkovQuantilePredictor):
    ...
    def predict_at_point(self, prices, volumes=None, idx=None, max_predictions=None, current_predictions=0):
        ...
        # Определяем квантили в соответствии с конфигурацией
        # Находим ближайшие к 0.05, 0.5 и 0.95 квантили из доступных
        available_quantiles = sorted(predictions.keys())
        
        # Находим ближайший квантиль к 0.05 (нижний экстремальный)
        lower_extreme_q = min(available_quantiles, key=lambda q: abs(q - 0.05))
        lower_extreme = predictions[lower_extreme_q]
        
        # Находим ближайший квантиль к 0.25 (нижний средний)
        lower_mid_q = min(available_quantiles, key=lambda q: abs(q - 0.25))
        lower_mid = predictions[lower_mid_q]
        
        # Находим ближайший квантиль к 0.5 (медиана)
        median_q = min(available_quantiles, key=lambda q: abs(q - 0.5))
        median = predictions[median_q]
        
        # Находим ближайший квантиль к 0.75 (верхний средний)
        upper_mid_q = min(available_quantiles, key=lambda q: abs(q - 0.75))
        upper_mid = predictions[upper_mid_q]
        
        # Находим ближайший квантиль к 0.95 (верхний экстремальный)
        upper_extreme_q = min(available_quantiles, key=lambda q: abs(q - 0.95))
        upper_extreme = predictions[upper_extreme_q]
        
        # Отладочная информация
        if idx % 1000 == 0:
            print(f"  Квантили: {lower_extreme_q}%={lower_extreme:.6f}, {lower_mid_q}%={lower_mid:.6f}, " +
                f"{median_q}%={median:.6f}, {upper_mid_q}%={upper_mid:.6f}, {upper_extreme_q}%={upper_extreme:.6f}")
        ...
        # *** ЗДЕСЬ ИЗМЕНЕН РАСЧЕТ УВЕРЕННОСТИ ***
        # Используем более простой и надежный подход для расчета уверенности
        if prediction == 1:  # Рост
            # Для роста: насколько все квантили выше порога
            # Чем больше квантилей выше порога, тем выше уверенность
            quantiles_above_threshold = sum(1 for q in [lower_extreme, lower_mid, median, upper_mid, upper_extreme] if q > dynamic_threshold)
            confidence = quantiles_above_threshold / 5.0  # От 0 до 1
        elif prediction == 2:  # Падение
            # Для падения: насколько все квантили ниже отрицательного порога
            # Чем больше квантилей ниже отрицательного порога, тем выше уверенность
            quantiles_below_neg_threshold = sum(1 for q in [lower_extreme, lower_mid, median, upper_mid, upper_extreme] if q < -dynamic_threshold)
            confidence = quantiles_below_neg_threshold / 5.0  # От 0 до 1
        else:
            confidence = 0.0
